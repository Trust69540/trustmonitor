...
export default function ChefProjetReportsScreen({ navigation }) {
  ...
  const loadReports = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/rapports?statut=VALIDE_PAR_SUPERVISEUR`);
  ...
  const patchRapport = async (id, action, commentaire) => {
    const res = await fetch(`${API_URL}/rapports/${id}`, {
      ...
      body: JSON.stringify({ action, role: "chef_projet", commentaire }),
    });
    return res;
  };
  ...
  const approve = async (rapport) => {
    ...
    try {
      ...
      showAlert("Validé", `Rapport #${rapport.id} validé définitivement.`);
      setSelected(null);
      loadReports();
    } catch (err) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Rapports validés par le superviseur</Text>

      {loading ? (
        <ActivityIndicator size="large" style={{ marginTop: 24 }} />
      ) : rapports.length === 0 ? (
        <Text style={styles.empty}>Aucun rapport en attente de validation finale</Text>
      ) : (
        <FlatList
          data={rapports}
